...
import { TrendingUp, Eye, MousePointer, Star, Pencil, Trash2 } from 'lucide-react';
...
export default function AdCard({ ad, delay = 0, onEdit, onDelete, isDeleting }: AdCardProps) {
    ...
    <div className="relative">
      <Link href={`/ads/${ad.id}`}>
        <div
          ...
          <div className="relative h-44 overflow-hidden">
            ...
            {/* Action buttons overlay — only if actions are provided */}
            {hasActions && (
              <div className="absolute bottom-3 left-3 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                {onEdit && (
                  <button
                    ...
                    className="flex items-center justify-center w-7 h-7 rounded-lg text-white transition-colors"
                ...
                {onDelete && (
                  <button
                    ...
                    className="flex items-center justify-center w-7 h-7 rounded-lg text-white hover:text-rose-300 transition-colors"
                    style={{ background: 'rgba(239,68,68,0.75)', backdropFilter: 'blur(8px)' }}
                    title="Delete ad"
                  >
                    <Trash2 size={12} />
                  </button>
      ...
      {isDeleting && (
        <div className="absolute inset-0 rounded-2xl flex items-center justify-center"
          style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 10 }}>
          <div className="w-7 h-7 rounded-full border-2 border-rose-400 border-t-transparent animate-spin" />
        </div>
      )}
    </div>
  );
